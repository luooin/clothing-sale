...
//与传统的 @Controller 不同，
// @RestController 不需要在每个处理方法上使用 @ResponseBody 注解
// ，因为它默认将所有返回值都转换为适当的数据格式（例如 JSON）并包含在响应体中。
//总之，@RestController 是一种方便的方式来创建 RESTful 风格的控制器，用于处理请求并返回数据
// ，无需额外的配置即可实现数据的自动转换和封装
@RestController
@Controller
